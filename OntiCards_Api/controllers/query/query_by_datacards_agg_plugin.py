"""
 @File: query_by_datacards_agg_plugin.py
 @Description: 聚合检索 - 适配ApiKey体系
...
def _llm_fuse_results(
    ...
    try:
        ...
        if analysis:
            ...
            if irrelevant:
                print(f"[agg] LLM融合分析: 排除不相关数据源 {irrelevant}")
            if relevant:
...
def _exec_cluster_parallel(cluster_idx: int, db_type: str, connect_info: dict,
    ...
    print(f"[_exec_cluster_parallel] 簇 {cluster_idx + 1} 开始执行...")
    ...
    try:
        ...
        elapsed_ms = int((time_module.time() - start_time) * 1000)
        print(f"[_exec_cluster_parallel] 簇 {cluster_idx + 1} 执行失败（ValueError）: {str(ve)}")
        error_result = {
            "db_type": db_type,
        ...
        print(f"[_exec_cluster_parallel] 簇 {cluster_idx + 1} 执行异常: {str(e)}")
        print(f"[_exec_cluster_parallel] 详细错误堆栈:\n{error_detail}")
...
def get_data_card_json(
    ...
    data_cards_list, vector_usage = search_vector(
        ...
        datasource_id=datasource_id  # 新增：传递数据源ID过滤（支持 str 或 list）
    )
...
def _exec_cluster(user_question: str, db_type: str, connect_info: str,
    ...
    print(f"[_exec_cluster] 开始执行，db_type={db_type}, 表数量={len(tables)}, 数据库={db_name}")

    # 始终使用数据库特定的提示词模板
    tpl = load_prompt(_pick_template_by_db(db_type))
    tables_block = make_tables_block(db_type, tables)
    print(f"[_exec_cluster] 表白名单块长度: {len(tables_block)} 字符")

    # ========== 【调试】打印实际传给LLM的表白名单 ==========
    print(f"[_exec_cluster] ========== 传给LLM的表白名单 ==========")
    for idx, t in enumerate(tables, 1):
        table_name = t.get("table_name", "unknown")
        col_count = len(t.get("columns", []))
        print(f"[_exec_cluster]   {idx}. 表名: {table_name}, 字段数: {col_count}")
    print(f"[_exec_cluster] ==========================================")
    # ========== 【调试结束】 ==========
    ...
    if has_relationship_cards:
        ...
        print(f"[_exec_cluster] ✅ 使用关系卡片增强的SQL生成")
        print(f"[_exec_cluster]   - 关系卡片数: {len(relationship_data.get('cards', {}))}")
        print(f"[_exec_cluster]   - JOIN建议数: {len(relationship_data.get('join_suggestions', []))}")
        print(f"[_exec_cluster]   - JOIN关系块长度: {len(rels_block)} 字符")
        print(f"[_exec_cluster]   - 关系卡片信息长度: {len(relationship_cards_info)} 字符")
        if relationship_cards_info:
            print(f"[_exec_cluster]   - 关系卡片信息预览:\n{relationship_cards_info[:500]}...")
    else:
        # 使用传统的JOIN关系推断
        rels_block = make_rels_block(tables)
        relationship_cards_info = ""  # 无关系卡片时传空字符串
        print(f"[_exec_cluster] ⚠️ 无关系卡片，使用传统JOIN推断")
        print(f"[_exec_cluster]   - JOIN关系块: {rels_block[:200] if rels_block else '(无)'}")
    ...
    # 先解析 LLM 输出的"类型"
    parsed = _extract_sql_from_llm_text(content)
    print(f"[_exec_cluster] LLM返回类型: {parsed['kind']}")
    ...
    if cte_names:
        print(f"[_exec_cluster] 检测到 CTE 名称: {cte_names}")
    ...
    if invalid_tables:
        print(f"[_exec_cluster] ⚠️ SQL包含非白名单表: {invalid_tables}")
        print(f"[_exec_cluster] 允许的表（规范化后）: {sorted(allowed_tables)}")
        print(f"[_exec_cluster] 允许的表（原始名称）: {[t.get('table_name') for t in tables]}")
        print(f"[_exec_cluster] CTE名称: {cte_names}")
        return {
            "db_type": db_type,
            "_connect_info_raw": connect_info,
            "connect_info_safe": {
    ...
    if not re.search(r"\bFROM\b", sql_text, re.IGNORECASE):
        print(f"[_exec_cluster] ⚠️ SQL缺少FROM子句，跳过执行")
        return {
            "db_type": db_type,
            "_connect_info_raw": connect_info,
            "connect_info_safe": {
    ...
    for attempt in range(max_retries + 1):
        ...
        try:
            ...
            if should_retry and attempt < max_retries:
                ...
                print(f"[_exec_cluster][{db_type}] 🔄 错误信息: {error_msg[:200]}...")
            ...
            if "does not exist" in error_msg:
                if "relation" in error_msg:
                    user_friendly_msg = "表不存在，可能是schema配置问题（PostgreSQL需要使用schema.table格式）"
...
def run_sql_safe_new(
    ...
    *,
    ...
    # ---------- 6) 执行 ----------
    sql_exec_start = time_module.time()
    with engine.connect() as conn:
        # Oracle 特殊处理：如果有 target_schema，需要先切换 session schema
        # 达梦 DM 与 Oracle 兼容，沿用同样逻辑（达梦支持 ALTER SESSION SET SCHEMA）
...
def _get_api_key_from_headers():
    """
    获取 API Key：
    1. Authorization: <api_key>
    2. Authorization: Bearer <api_key>（兼容）
    3. X-API-Key: <api_key>
    """
...
def _is_expired(expires_at):
    ...
    # expires_at 如果是 naive，当作 UTC（兜底）
...
def _require_api_key_user_id():
    ...
    返回: (user_id, error_tuple, api_key_id)
        ...
        - api_key_id: API Key 的 ID
    """
    k = _get_api_key_from_headers()
    if not k:
        return None, ("缺少 API Key", 401), None

    obj = ApiKey.query.filter_by(api_key=k).first()
    if not obj:
        return None, ("API Key 无效", 401), None

    if obj.status != "active":
        return None, ("API Key 已禁用", 403), None

    print("expires_at:", obj.expires_at, "tzinfo:", getattr(obj.expires_at, "tzinfo", None))
    if _is_expired(obj.expires_at):
        return None, ("API Key 已过期", 403), None
...
class QueryByDataCardsAggPlugin(Resource):
    def post(self):
        # 获取请求头中的api key 并校验
        user_id, err, api_key_id = _require_api_key_user_id()
        print(f"[AGG][AUTH] api_key user_id = {user_id}, api_key_id = {api_key_id}")

        if err:
            msg, http_code = err
            return format_response(None, http_code, msg)
        ...
        # 获取可选参数：数据源名称过滤（支持通过数据源名称获取ID）
        # 注意：必须在术语展开之前提取这些参数，因为术语展开逻辑需要使用它们
        connect_name = body.get("connect_name")
        datasource_id = body.get("datasource_id")
        datasource_ids_param = body.get("datasource_ids")  # 支持传入数据源ID列表

        # 如果传入了数据源名称(connect_name)，则根据名称查找对应的数据源ID
        if connect_name:
        ...
        recall_doc_ids = []  # 召回到的全部 doc_id
        owned_doc_ids = []  # 属于当前 user 的 doc_id（筛选后）
        filtered_out_doc_ids = []  # 不属于当前 user 的 doc_id（被过滤掉）
        for doc_id, card in zip(doc_ids, card_list):
            ...
            if not schema_row:
                filtered_out_doc_ids.append(doc_id)
                continue

            owned_doc_ids.append((doc_id, schema_row.table_name))

            if not schema_row:
                continue

            connect_name = ((card or {}).get("DocInfo") or {}).get("connect_name")
            connect_info = map_connect_name_to_connect_info(connect_name, DatasourceInfo, user_id=user_id)
            if not connect_info:
        ...
        # 用户隔离日志打印
        print("\n" + "=" * 80)
        print(f"[DEBUG][CARDS] request_user_id = {user_id}")
        print(f"[DEBUG][CARDS] recall_total = {len(recall_doc_ids)}")
        print(f"[DEBUG][CARDS] recall_doc_ids = {recall_doc_ids}")

        print(f"[DEBUG][CARDS] owned_total = {len(owned_doc_ids)}")
        print(f"[DEBUG][CARDS] owned_doc_ids = {[x[0] for x in owned_doc_ids]}")
        print(f"[DEBUG][CARDS] owned_doc_id->table = {owned_doc_ids}")

        print(f"[DEBUG][CARDS] filtered_out_total = {len(filtered_out_doc_ids)}")
        ...
        if source_datasource_ids:
            for ds_id in source_datasource_ids:
                ds_info = DatasourceInfo.query.filter_by(id=ds_id).first()
                if ds_info:
                    ds_name = ds_info.database_name or ds_info.connect_name
